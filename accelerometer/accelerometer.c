...
#define X_LOW_VOLTS 1.34
#define Y_LOW_VOLTS 1.32
#define X_HIGH_VOLTS 1.98
#define Y_HIGH_VOLTS 1.95

#define LOW_FILTER 2.0
#define HIGH_FILTER 2.0
...
void main (void)
{
	...
	InitPinADC(0, 2); // Configure P2.3 as analog input
    InitADC();
	...
	while(1)
	{
		...
		x_volts = 16.0 * (v[0] - MID_VOLTS) / (X_HIGH_VOLTS - X_LOW_VOLTS);
		y_volts = -16.0 * (v[1] - MID_VOLTS) / (Y_HIGH_VOLTS - Y_LOW_VOLTS);
		
		// Filter
		if (x_volts < LOW_FILTER && x_volts > -LOW_FILTER) x_volts = 0;
		if (y_volts < LOW_FILTER && y_volts > -LOW_FILTER) y_volts = 0;
		
		if (x_volts > HIGH_FILTER) x_volts = HIGH_FILTER;
		else if (x_volts < -HIGH_FILTER) x_volts = -HIGH_FILTER;
		if (y_volts > HIGH_FILTER) y_volts = HIGH_FILTER;
		else if (y_volts < -HIGH_FILTER) y_volts = -HIGH_FILTER;
		
		// Round
		x = x_volts;
		y = y_volts;
		
		printf ("V@xPin=%7.5fV, V@yPin=%7.5fV, x_volts=%f, y_volts=%f, x=%d, y=%d,   \r", v[0], v[1], x_volts, y_volts, x, y);
		waitms(500);
	 }  
}	
